...
async function create(req,res)
{
    ...
    try
    {
        ...
        else if(p.length>=8)
        {
            ...
            if(b==1)
                {
                    res.send('<script>alert("Password must contain atleast one special characters");location.href=("http://localhost:7000/register");</script>');
                }
...
// db.collection('Login').insertOne({First_Name:firstname,Last_Name:lastname,Email:email,Gender:gender,Contact:phone,Age:age,Username:username,Password:hashedPassword})